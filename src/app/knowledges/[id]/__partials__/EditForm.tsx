...
export function EditForm() {
  ...
  const { data: knowledge, isLoading } = useGetKnowledgeDetail({
    id: id as string,
  });
  ...
  const onSubmit = async (values: FormValues) => {
    updateKnowledge(
      ...
      {
        onSuccess: () => {
          message.success("Pengetahuan berhasil diubah");
          router.push("/knowledges");
        },
        onError: () => {
          message.error("Gagal mengubah pengetahuan");
        },
      }
    );
  };
  ...
  useEffect(() => {
    if (knowledge) {
      setValue("keywords", knowledge.keywords);
      setValue("answer", knowledge.answer);
      setValue("category", knowledge.category);
    }
  }, [knowledge, setValue]);
